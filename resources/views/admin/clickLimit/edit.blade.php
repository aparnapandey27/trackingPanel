...
@section('title', 'Edit Student Click Limit')
        ...
        <div class="container-fluid">
            ...
            <div class="card mb-4 shadow-lg border-left-dark">
                <div class="card-header with-border">
                    <h6 class="card-title mb-0 flex-grow-1 text-primary">Student Click & Conversion Limit Details</h6>
                    <!-- /.card-tools -->
                </div>
                <!-- /.card-header -->
                <div class="card-body">
                    <form action="{{ route('admin.offers.clicklimit.update', [$clicklimit->offer_id, $clicklimit->id]) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="form-group row mb-3" id="student_id">
                            ...
                            <select class="form-control student" data-placeholder="Choose any Student to Generate its Click Limit" >
                                <option value="selectedStudentId">{{$clicklimit->user->name}}</option>
                                </select>
                        ...
                        <div class="form-group row mb-4" id="clicklimit_group">
                            ...
                            <div class="col-sm-9">
                            <input type="number" pattern="^[0-9]{1,5}" name="click_limit" class="form-control" id="click_limit"  inputmode="numeric" maxlength="5" value="{{ $clicklimit->clicklimit }}">
                        ...
                        <div class="text-center mb-3">
                            <button type="submit" class="btn btn-rounded btn-primary"><i class="fa fa-check-circle"></i>
                                Update Student Offer Click Limit
                            </button>
                        </div>                                                              
                    </form>
...
@push('scripts')
    ...
    <script>
          var selectedStudentId = {{$clicklimit->student_id}}; // Declare the variable to store the 
