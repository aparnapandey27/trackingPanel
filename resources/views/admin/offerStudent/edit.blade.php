...
@section('title', 'Edit Student Offer Payout')
@push('style')
 <!-- Sweetalert2 -->
    ...
    <div class="page-content">
        <div class="container-fluid">
            ...
            <div class="card mb-4 shadow-lg border-left-dark">
                ...
                <div class="card-body">
                    <form action="{{ route('admin.offer.student.update', [$offerStudent->offer_id, $offerStudent->id]) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="form-group row mb-3" id="student_id">
                            ...
                            <select class="form-control student" data-placeholder="Choose any Student to Generate its Payout" >
                                <option value="{{ $offerStudent->student_id }}">[{{ $offerStudent->student_id }}] {{ $offerStudent->user->name }}</option>
                        ...
                        <div class="form-group row mb-3">
                            ...
                            <div class="col-sm-9 col-md-6">
                                <select class="form-select" name="goal_name" id="goal_name">
                                    <option value="Default" data-pay-model="{{ $offer->offer_model }}" data-currency="{{ $offer->currency }}" data-default-payout="{{ $offer->default_payout }}" data-percent-payout="{{ $offer->percent_payout }}" 
                                    {{ $offerStudent->name == 'Default' ? 'selected' : '' }}>Default</option>
                                    @foreach ($offer->goals as $goal)
                                    <option value="{{ $goal->goalName->name }}" data-pay-model="{{ $goal->offer_model }}" data-currency="{{ $goal->currency }}" data-default-payout="{{ $goal->default_payout }}" data-percent-payout="{{ $goal->percent_payout }}"
                                    {{ $offerStudent->name == $goal->goalName->name ? 'selected' : '' }} >{{ $goal->goalName->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="form-group row mb-3" id="pay_model_group">
                            ...
                            <div class="col-sm-9 col-md-6">
                                <input name="payModel" class="form-control" id="payModel" data-placeholder="Goal Model for particular Event"  value="{{ $offerStudent->pay_model }}">
                            </div>
                        </div>
                        ...
                        <div class="form-group row mb-3" id="currency_group">
                            ...
                            <div class="col-sm-9 col-md-6">
                                <input disabled="disabled" name="currency"  class="form-control" id="currency" data-placeholder="Currency for particular Event" value="{{ $offerStudent->currency }}">
                        ...
                        <div class="form-group row mb-3" id="defaultPayout_group">
                            ...
                            <div class="col-sm-9 col-md-6">
                            <input type="number" pattern="^[0-9]{1,2}(\.[0-9]{0,6})?$" name="defaultPayout" class="form-control" id="defaultPayout" value="{{ $offerStudent->default_payout }}">
                    ...
                    <div class="form-group row mb-3" id="percentPayout_group">
                            ...
                            <div class="col-sm-9 col-md-6">
                                ...
                                <input type="number" pattern="^[0-9]{1,2}(\.[0-9]{0,6})?$" name="percentPayout" class="form-control" id="percentPayout" step="0.001" value="{{ $offerStudent->percent_payout}}">
...
@push('scripts')
    ...
    <script>
          var selectedStudentId = {{ $offerStudent->student_id }}; // Declare the variable to store the 
        var payModel = null;
        var currency = null;
